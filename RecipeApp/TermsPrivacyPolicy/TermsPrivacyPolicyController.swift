...
class TermsPrivacyPolicyView: UIView {
    ...
    /// Cостояние TermsPrivacyPolicyView
    enum StateView {
        case openView
        case closeView
    }
    ...
    private func setupView() {
        let tapGestureRecognizer = UITapGestureRecognizer(
            target: self,
            action: #selector(viewTap(gestureRecognizer:))
        )
        let panGestureRecognizer = UIPanGestureRecognizer(
            target: self,
            action: #selector(viewPan(gestureRecognizer:))
        )
    ...
    private func setupConstraints() {
        ...
        makeHandleViewConstraint()
        makeLogoLabelConstraints()
        makeTextLabelConstraint()
    ...
    private func makeHandleViewConstraint() {
        handleView.topAnchor.constraint(equalTo: topAnchor, constant: 15).isActive = true
        handleView.centerXAnchor.constraint(equalTo: centerXAnchor).isActive = true
    }

    private func makeLogoLabelConstraints() {
        logoLabel.topAnchor.constraint(equalTo: topAnchor, constant: 55).isActive = true
        logoLabel.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 25).isActive = true
    }

    private func makeTextLabelConstraint() {
