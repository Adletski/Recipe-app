// Service.swift
// Copyright © RoadMap. All rights reserved.

import Foundation

// MARK: - Types
/// создание модели с данными
final class Service {
    var description = """
    1/2 to 2 fish heads, depending on size, about 5 pounds total
    2 tablespoons vegetable oil
    1/4 cup red or green thai curry paste
    3 tablespoons fish sauce or anchovy sauce
    1 tablespoon sugar
    1 can coconut milk, about 12 ounces
    3 medium size asian eggplants, cut int 1 inch rounds
    Handful of bird's eye chilies
    1/2 cup thai basil leaves
    Juice of 3 limes
    1/2 to 2 fish heads, depending on size, about 5 pounds total
    2 tablespoons vegetable oil
    1/4 cup red or green thai curry paste
    3 tablespoons fish sauce or anchovy sauce
    1 tablespoon sugar
    1 can coconut milk, about 12 ounces
    3 medium size asian eggplants, cut int 1 inch rounds
    Handful of bird's eye chilies
    1/2 cup thai basil leaves
    Juice of 3 limes
    1/2 to 2 fish heads, depending on size, about 5 pounds total
    2 tablespoons vegetable oil
    1/4 cup red or green thai curry paste
    3 tablespoons fish sauce or anchovy sauce
    1 tablespoon sugar
    1 can coconut milk, about 12 ounces
    3 medium size asian eggplants, cut int 1 inch rounds
    Handful of bird's eye chilies
    1/2 cup thai basil leaves
    Juice of 3 limes
    """

    // MARK: - Public Methods

    /// Получения списка категорий блюд и их характеристики
    func getCategoryList() -> [FoodModel] {
        [
            FoodModel(
                image: "food1",
                name: "Simple Fish And Corn",
                time: "60 min",
                timeCount: 60,
                kkal: "274 kkal",
                kkalCount: 274,
                weight: 793,
                carbohydrates: 10.78,
                fats: 10.00,
                proteins: 97.30,
                descriptions: description
            ),
            FoodModel(
                image: "food2",
                name: "Fast Roast Fish & Show Peas Recipes",
                time: "80 min",
                timeCount: 80, kkal: "94 kkal",
                kkalCount: 94,
                weight: 793,
                carbohydrates: 10.78,
                fats: 10.00,
                proteins: 97.30,
                descriptions: description
            ),
            FoodModel(
                image: "food3",
                name: "Lemon and Chili Fish Fish Burrito",
                time: "90 min",
                timeCount: 90, kkal: "226 kkal",
                kkalCount: 226, weight: 793,
                carbohydrates: 10.78,
                fats: 10.00,
                proteins: 97.30,
                descriptions: description
            ),
            FoodModel(
                image: "food4",
                name: "Baked Fish with Lemon Herb Sauce",
                time: "90 min",
                timeCount: 90, kkal: "616 kkal",
                kkalCount: 616, weight: 793,
                carbohydrates: 10.78,
                fats: 10.00,
                proteins: 97.30,
                descriptions: description
            ),
            FoodModel(
                image: "food5",
                name: "Salmon with Cantaloupe and Fried Shallots",
                time: "100 min",
                timeCount: 100, kkal: "410 kkal",
                kkalCount: 410, weight: 793,
                carbohydrates: 10.78,
                fats: 10.00,
                proteins: 97.30,
                descriptions: description
            ),
            FoodModel(
                image: "food6",
                name: "Chili and Tomato Fish",
                time: "100 min",
                timeCount: 100, kkal: "174 kkal",
                kkalCount: 174, weight: 793,
                carbohydrates: 10.78,
                fats: 10.00,
                proteins: 97.30,
                descriptions: description
            )
        ]
    }
}
